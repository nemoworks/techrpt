...
\begin{tabular}{ccc}
  ...
  Operation& []ResourceOperationSpec& 资源操作列表\\
  ProbeImage& string& 探针的镜像\\
  ProbeArgs& []string& 启动探针的命令行参数\\
  ProbeEnabled& bool& 是否启动探针\\
  AliasName& string& 资源别名\\
  \bottomrule
\end{tabular}


ResourceOperationSpec结构如下：
...
\begin{tabular}{ccc}
  ...
  \midrule
  Name& string& 操作名称\\
  Price& float64& 操作价格，比如操作咖啡机就是一杯咖啡的价格\\
  Description& string& 操作描述\\
  HTTPAction& HTTPActionSpec& 操作接口\\
  \bottomrule
\end{tabular}
